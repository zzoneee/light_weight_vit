    ...
    def __init__(self,
        ...
        if num_downs > 0:
            ...
            self.wsas = nn.ModuleList([
                ...
                ConvBNReLU(dim, dim, kernel_size, 1, groups=dim) for i in range(num_downs)
            ])
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, use_sa_global=True, num_tokens=1, add_global_rate=0.5, window_size=2,
                 ...
                 use_proj_global=False, kernel_size=3):
        super().__init__()
        self.num_tokens = num_tokens
...
class LightViT_DualToken_conv(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=8, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
        ...
        # self.pos_embeds = []
        for stage, (embed_dim, num_layer, num_head, mlp_ratio, kernel_size) in enumerate(
                zip(embed_dims, num_layers, num_heads, mlp_ratios, ks)):
...
@register_model
def lightvit_dualToken_left_tiny_conv(pretrained=False, **kwargs):
...
@register_model
def lightvit_dualToken_left_tiny_conv35(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_left_tiny_conv55(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_left_tiny_conv57(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
