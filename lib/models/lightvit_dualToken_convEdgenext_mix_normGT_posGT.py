...
class LightViT_DualToken_convEdgenext_mix_normGT_posGT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=8, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, self.num_tokens, embed_dims[0]))
    ...
    def no_weight_decay(self):
        return {'global_token', '[g]relative_position_bias_table', 'pos_embed'}

    def forward_features(self, x):
        x, (H, W) = self.patch_embed(x)
        global_token = self.global_token.expand(x.shape[0], -1, -1)
        global_token = global_token + self.pos_embed
        # x = torch.cat((global_token, x), dim=1)
        # stage_cnt = 0
...
@register_model
def lightvit_dualToken_left_tiny_convEdgenext_mix_normGT_posGT(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        **kwargs)
    model = LightViT_DualToken_convEdgenext_mix_normGT_posGT(**model_kwargs)
    return model
