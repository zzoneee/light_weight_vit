...
class LightViT_DualToken_convEdgenext_mix_normGT_posCos(nn.Module):
    ...
    def no_weight_decay(self):
        return {'[g]relative_position_bias_table'}

    def forward_features(self, x):
        x, (H, W) = self.patch_embed(x)
        # global_token = self.global_token.expand(x.shape[0], -1, -1)
        pos_encoding = PositionalEncoding(x.shape[2], 0, self.num_tokens)
        pos_encoding.eval()
        pos_embed = pos_encoding.P[:, :self.num_tokens, :]
        global_token = pos_embed.to(x.device)
...
@register_model
def lightvit_dualToken_left_tiny_convEdgenext_mix_normGT_posCos(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        **kwargs)
    model = LightViT_DualToken_convEdgenext_mix_normGT_posCos(**model_kwargs)
    return model
