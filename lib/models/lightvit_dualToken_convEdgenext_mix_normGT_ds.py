...
class Attention(nn.Module):
    ...
    def __init__(self,
        ...
        if num_downs == 1:
            ...
            self.down_1_sampling = nn.AvgPool2d(kernel_size=2, stride=2)
        elif num_downs == 2:
            # self.norm_2_down_sampling = norm_layer(dim)
            # self.act_2_down_sampling = act_layer()
            self.down_2_sampling = nn.AvgPool2d(kernel_size=4, stride=4)
...
class LightViT_DualToken_convEdgenext_mix_normGT_ds(nn.Module):
...
@register_model
def lightvit_dualToken_left_tiny_convEdgenext_mix_normGT_ds(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        # hydras=[False, False, True],
                        **kwargs)
    model = LightViT_DualToken_convEdgenext_mix_normGT_ds(**model_kwargs)
    return model
