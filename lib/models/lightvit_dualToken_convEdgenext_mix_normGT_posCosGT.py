...
class LightViT_DualToken_convEdgenext_mix_normGT_posCosGT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        global_token = global_token + pos_embed.to(global_token.device)
...
@register_model
def lightvit_dualToken_left_tiny_convEdgenext_mix_normGT_posCosGT(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        **kwargs)
    model = LightViT_DualToken_convEdgenext_mix_normGT_posCosGT(**model_kwargs)
    return model
