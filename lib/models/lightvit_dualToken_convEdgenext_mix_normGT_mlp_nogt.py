...
class Attention(nn.Module):
    ...
    def forward(self, x, global_token):
        ...
        x_global = self.forward_global_broadcast(x_origin, x_downsampleToken)
...
class Block(nn.Module):
    ...
    def forward(self, x, global_token, H, W):
        ...
        x_, _ = self.attn(self.norm1(x), global_token)
...
class LightViT_DualToken_convEdgenext_mix_normGT_mlp_nogt(nn.Module):
...
# @register_model
# def lightvit_dualToken_left_xxtiny_convEdgenext_mix_normGT_mlp(pretrained=False, **kwargs):
#     model_kwargs = dict(patch_size=8,
#                         embed_dims=[40, 80, 160],
#                         num_layers=[1, 4, 3],
#                         num_heads=[2, 4, 8],
#                         mlp_ratios=[8, 4, 4],
#                         use_sa_global=False,
...
#                         **kwargs)
#     model = LightViT_DualToken_convEdgenext_mix_normGT_mlp(**model_kwargs)
#     return model
...
@register_model
def lightvit_dualToken_left_tiny_convEdgenext_mix_normGT_mlp_nogt(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        # hydras=[False, False, True],
                        **kwargs)
    model = LightViT_DualToken_convEdgenext_mix_normGT_mlp_nogt(**model_kwargs)
    return model
...
# >>> 0.058+0.003+0.007+0.916+0.001
# 0.985

# Number of backbone parameters: 10.8 M
# Number of backbone parameters requiring grad: 10.8 M
# Number of total parameters: 10.8 M
# Number of total parameters requiring grad: 10.8 M
