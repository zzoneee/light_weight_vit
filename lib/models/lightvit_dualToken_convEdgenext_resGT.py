...
class Attention(nn.Module):
    ...
    def forward(self, x, global_token):
        ...
        if self.use_sa_global:
            ...
            global_token = global_token + self.Global_Proj(global_token)
...
class LightViT_DualToken_convEdgenext_resGT(nn.Module):
...
@register_model
def lightvit_dualToken_left_tiny_convEdgenext_resGT(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        **kwargs)
    model = LightViT_DualToken_convEdgenext_resGT(**model_kwargs)
    return model
