    ...
    def __init__(self,
        ...
        if num_downs > 0:
            ...
            self.wsas = nn.ModuleList([
                ...
                ConvBNReLU(dim, dim, 3, 1, 1) for i in range(num_downs)
            ])
...
class PatchEmbed(nn.Module):
    ...
    # def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
    def __init__(self, dim=3, out_dim=768, num_tokens=1, patch_size=2):
        super().__init__()
        self.num_tokens = num_tokens
        ...
        self.proj = nn.Conv2d(
            dim, out_dim, kernel_size=patch_size, stride=patch_size)

        self.norm2 = nn.LayerNorm(dim)
        self.proj_global_token = nn.Linear(dim, out_dim, bias=False)

    def forward(self, x, global_token, HH, WW):
        # B, C, H, W = x.shape
        # assert H == self.img_size[0] and W == self.img_size[1], \
        #     f"Input image size ({B}*{C}*{H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."

        B, L, C = x.shape
        x = x.view(B, HH, WW, C).permute(0, 3, 1, 2).contiguous()
        ...
        global_token = self.proj_global_token(self.norm2(global_token))
        return x, global_token, (HH // 2, WW // 2)


# @BACKBONES.register_module
class LightViT_DualToken_conv(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=8, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
                 ...
                 num_downs=[2, 1, 0], window_size_backbone=2, use_proj_global=False):
        super().__init__()
        ...
        for stage, (embed_dim, num_layer, num_head, mlp_ratio) in enumerate(
            ...
            if stage > 0:
                # downsample
                blocks.append(PatchEmbed(embed_dims[stage - 1], embed_dim, num_tokens=num_tokens))
            blocks += [
                Block(
...
@register_model
def lightvit_dualToken_left_tiny_conv_convMergePatch(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        # global_ffn=False,
                        **kwargs)
    model = LightViT_DualToken_conv(**model_kwargs)
    return model
