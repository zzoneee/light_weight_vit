...
class Attention(nn.Module):
    ...
    def forward(self, x, global_token):
        ...
        if self.num_downs > 0:
            ...
            x_local = self.forward_global_aggregation(x)
        ...
        global_token = self.forward_updata_globalToken(global_token, x_origin)
...
class Block(nn.Module):
    ...
    def forward(self, x, global_token, H, W):
        ...
        x_, global_token_ = self.attn(self.norm1(x), self.normGT(global_token))
...
class LightViT_DualToken_convEdgenext_mlp_normGT_nodsGT8(nn.Module):
...
@register_model
def lightvit_dualToken_left_tiny_convEdgenext_mlp_normGT_nodsGT8(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        # hydras=[False, False, True],
                        **kwargs)
    model = LightViT_DualToken_convEdgenext_mlp_normGT_nodsGT8(**model_kwargs)
    return model
...
# Number of backbone parameters: 13.283 M
# Number of backbone parameters requiring grad: 13.283 M
# Number of total parameters: 13.283 M
# Number of total parameters requiring grad: 13.283 M
# new: 12.961
