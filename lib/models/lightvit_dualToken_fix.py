...
class Block(nn.Module):
    ...
    def forward(self, x, global_token, H, W):
        ...
        x_, global_token_ = self.attn(self.norm1(x), global_token)
...
class LightViT_DualToken_fix(nn.Module):

    def __init__(self, img_size=224, patch_size=8, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
                 num_layers=[2, 6, 6], num_heads=[2, 4, 8], mlp_ratios=[8, 4, 4], use_sa_global=False, num_tokens=8,
                 ...
                 num_downs=[2, 1, 0], window_size_backbone=2, use_proj_global=False, global_proj='left'):
        super().__init__()
        ...
        for stage, (embed_dim, num_layer, num_head, mlp_ratio) in enumerate(
            ...
            blocks += [
                Block(
                    ...
                    window_size_backbone=window_size_backbone, use_proj_global=use_proj_global)
...
@register_model
def lightvit_dualToken_tiny_fix(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        **kwargs)
    model = LightViT_DualToken_fix(**model_kwargs)
    return model


@register_model
def lightvit_dualToken_left_tiny_fix(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_up_tiny_fix(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_right_tiny_fix(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_left_add_tiny_fix(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_up_add_tiny_fix(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_right_add_tiny_fix(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
