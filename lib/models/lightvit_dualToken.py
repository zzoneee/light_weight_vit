...
class Attention(nn.Module):
    ...
    def __init__(self,
        ...
        # self.norm_up_sampling = [norm_layer(dim) for i in range(num_downs)]
        # self.act_up_sampling = [act_layer() for i in range(num_downs)]
        # # self.proj_up_sampling = [nn.Linear(dim, dim) for i in range(num_downs)]

        # if num_downs >= 1:
        #     self.norm_1_up_sampling = norm_layer(dim)
        #     self.act_1_up_sampling = act_layer()
        #     self.proj_1_up_sampling = nn.Linear(dim, dim)
        #     self.anti_1_aliasing = nn.Sigmoid()
        #     self.up_1_sampling = nn.Upsample(scale_factor=2, mode="bilinear")
        # if num_downs >= 2:
        #     self.norm_2_up_sampling = norm_layer(dim)
        #     self.act_2_up_sampling = act_layer()
        #     self.proj_2_up_sampling = nn.Linear(dim, dim)
        #     self.anti_2_aliasing = nn.Sigmoid()
        #     self.up_2_sampling = nn.Upsample(scale_factor=2, mode="bilinear")

        # if num_downs >= 3:
        #     self.proj_3_up_sampling = nn.Linear(dim, dim)
        # self.anti_aliasing = [nn.Sigmoid() for i in range(num_downs)]
        # self.up_sampling = [nn.Upsample(scale_factor=2 ** (i + 1), mode="nearest") for i in range(num_downs)]
        # self.up_sampling = [nn.Upsample(scale_factor=2, mode="bilinear") for i in range(num_downs)]
        ...
        #     self.down_sampling = nn.AvgPool2d(kernel_size=window_size, stride=window_size)

        self.attn_drop = nn.Dropout(attn_drop) if attn_drop > 0 else nn.Identity()

        self.ga_qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        # self.ga_proj = nn.Linear(dim, dim)
        ...
        # self.gb_proj = nn.Linear(dim, dim)
    ...
    def forward(self, x):
        ...
        if self.num_downs > 0:
            ...
            for wsa in self.wsas:
                ...
                if len(x_multi_scale) == 0:
                    # x_out = x.reshape(B, N, C)

                    # x_multi_scale.append(x.reshape(B, N, C))

                    x_multi_scale.append(x)
                else:
                    # x_out += self.up_sampling[up_sampling_cnt](x).reshape(B, N_origin, C)

                    # x_multi_scale.append(self.up_sampling[up_sampling_cnt](x).reshape(B, N_origin, C))
                    # up_sampling_cnt += 1
            ...
            x = self.forward_global_aggregation(x)
            _, N, _ = x.shape
            H = W = int(N ** 0.5)
            # x_multi_scale.append(self.up_sampling[up_sampling_cnt](x.reshape(B, H, W, C).permute(0, 3, 1, 2)).reshape(B, N_origin, C))
            # x_out = sum(x_multi_scale) + self.forward_global_broadcast(x_origin, x)

            x_multi_scale.append(x.reshape(B, H, W, C))

            # for i in range(self.num_downs, 0, -1):  # from self.num_downs to 1
            #     if i == self.num_downs:
            ...
            #         pos_embed = None
            #
            #     # x_multi_scale[i - 1] = x_multi_scale[i - 1] + self.up_sampling[i - 1](self.anti_aliasing[i - 1](pos_embed) + self.proj_up_sampling[i - 1](self.act_up_sampling[i - 1](self.norm_up_sampling[i - 1](x_multi_scale[i]))))
            #     # x_tmp = self.act_up_sampling[i - 1](self.norm_up_sampling[i - 1](x_multi_scale[i]))
            #     # x_tmp = self.proj_up_sampling[i - 1](x_tmp)
            #     if i == 1:
            #         x_tmp = self.act_1_up_sampling(self.norm_1_up_sampling(x_multi_scale[i]))
            #         x_tmp = self.proj_1_up_sampling(x_tmp)
            #     else:
            #         x_tmp = self.act_2_up_sampling(self.norm_2_up_sampling(x_multi_scale[i]))
            #         x_tmp = self.proj_2_up_sampling(x_tmp)
            #     # else:
            #     #     x_tmp = self.proj_3_up_sampling(x_tmp)
            #     if pos_embed is not None:
            #         if i == 1:
            #             anti_aliasing = self.anti_1_aliasing(pos_embed)
            #         else:
            #             anti_aliasing = self.anti_2_aliasing(pos_embed)
            #         x_pre = x_tmp.reshape(B, -1, C) + anti_aliasing
            #     else:
            #         x_pre = x_tmp.reshape(B, -1, C)
            #
            #     _, N, _ = x_pre.shape
            #     H = W = int(N ** 0.5)
            #     if i == 1:
            #         x_multi_scale[i - 1] = x_multi_scale[i - 1] + self.up_1_sampling(x_pre.reshape(B, H, W, C).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            #     else:
            #         x_multi_scale[i - 1] = x_multi_scale[i - 1] + self.up_2_sampling(
            #             x_pre.reshape(B, H, W, C).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)

            # x_out = x_multi_scale[0].reshape(B, -1, C) + self.forward_global_broadcast(x_origin, x)
            x_downsampleToken = x
            x_local = x_multi_scale[0].reshape(B, -1, C)
        else:
        ...
        if self.use_sa_global:
            ...
            if self.global_proj == 'left':
                global_token = self.Global_Proj(global_token)
            elif self.global_proj == 'up':
                x_downsampleToken = self.Global_Proj(x_downsampleToken)

            if self.add_global_rate >= 0:
                global_token = self.add_global_rate * global_token + (1 - self.add_global_rate) * x_downsampleToken
            else:
                global_token = global_token + x_downsampleToken

            if self.global_proj == 'right':
                global_token = self.Global_Proj(global_token)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, use_sa_global=True, num_tokens=1, add_global_rate=0.5, window_size=2,
                 ...
                 use_proj_global=False, global_proj=None, global_ffn=True):
        super().__init__()
        ...
        self.mlp = BiAttnMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                             num_tokens=num_tokens, global_ffn=global_ffn)
        self.global_ffn = global_ffn
...
class LightViT_DualToken(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=8, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
                 ...
                 num_downs=[2, 1, 0], window_size_backbone=2, use_proj_global=False, global_proj=None, global_ffn=True):
        super().__init__()
        ...
        for stage, (embed_dim, num_layer, num_head, mlp_ratio) in enumerate(
            ...
            blocks += [
                Block(
                    ...
                    window_size_backbone=window_size_backbone, use_proj_global=use_proj_global, global_proj=global_proj,
                    global_ffn=global_ffn)
...
@register_model
def lightvit_dualToken_tiny(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        ...
                        **kwargs)
    model = LightViT_DualToken(**model_kwargs)
    return model


@register_model
def lightvit_dualToken_left_tiny(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_up_tiny(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_right_tiny(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_left_add_tiny(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_up_add_tiny(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
...
@register_model
def lightvit_dualToken_right_add_tiny(pretrained=False, **kwargs):
    model_kwargs = dict(patch_size=8,
                        embed_dims=[64, 128, 256],
                        num_layers=[2, 6, 6],
                        num_heads=[2, 4, 8],
